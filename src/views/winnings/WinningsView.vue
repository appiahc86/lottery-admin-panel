...
const search = async () => {
  try {
    ...
    const response = await  axios.post('/admin/tickets/winners',
    ...
    if (response.status === 200){
      ...
      winningNumbers.value = response.data.winningNumbers;
...
const total = computed(() => {
  ...
  if (winners.value.length){
    for (const record of winners.value) {
      total += parseFloat(record.amountWon) || 0;
    }
  }
...
<template>
  <div class="container mt-2 mb-3">
    <div class="row justify-content-center">
      <h5 class="text-center mb-3">List Winners By Date</h5>
    ...
    <div class="row">
      <div class="col">
        ...
        <div class="table-responsive mt-3 mb-5">
          <!--        Table -->
          <DataTable :value="winners" :paginator="true" :rows="10" dataKey="id" :loading="loading"
            ...
            <Column field="numbers" header="Numbers" :sortable="false" class="data-table-font-size">
              <template #body="{data}">
                <td v-html="getWinningNumbers(JSON.parse(data.numbers), winningNumbers ? JSON.parse(winningNumbers) : null )"></td>
              </template>
            </Column>
            ...
            <Column field="amountWon" header="Won" :sortable="false" class="data-table-font-size">
              <template #body="{data}">
                <td>
                  {{ formatNumber(data.amountWon) }}
                </td>
              </template>
            </Column>
        ...
        <h5 class="">Total Winnings: <b>GH¢ {{ formatNumber(total) }}</b></h5>
